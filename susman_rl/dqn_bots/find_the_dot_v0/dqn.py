...
_train_env._env._envs[0].uuid = 'Training...'
print('Action Spec:', _train_env.action_spec())
...
avg_return = compute_avg_return(_eval_env, agent.policy, _num_eval_episodes)
returns = [avg_return]
...
while True:
    ...
    for _ in tqdm(range(_train_steps)):
        for _ in range(_collect_steps_per_iteration):
            collect_step(_train_env, agent.collect_policy, replay_buffer)
